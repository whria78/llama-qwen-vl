...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--onnx-threshold"}, "N",
        "onnx-threshold (default:0.5)",
        [](common_params & params, const std::string & value) {
            params.onnx_threshold = std::stof(value);
        }
    ).set_examples({LLAMA_EXAMPLE_LLAVA}));
    ...
    add_opt(common_arg(
        {"--copy-for-test"},
        "copy images to check the name & ID",
        [](common_params & params) {
            params.copy_for_test = true;
        }
    ).set_examples({LLAMA_EXAMPLE_LLAVA}));

    add_opt(common_arg(
        {"--organize-photo"},
        "automatically organize clinical photographs",
        [](common_params & params) {
            params.organize_photo = true;
        }
    ).set_examples({LLAMA_EXAMPLE_LLAVA}));

    add_opt(common_arg(
        {"--organize-photo-timegap"}, "N",
        "time gap (default:1200 = 20 minuts)",
        [](common_params & params, const std::string & value) {
            params.organize_photo_timegap = std::stof(value);
        }
    ).set_examples({LLAMA_EXAMPLE_LLAVA}));
...
*/
    ...
    ).set_examples({LLAMA_EXAMPLE_IMATRIX, LLAMA_EXAMPLE_CVECTOR_GENERATOR, LLAMA_EXAMPLE_EXPORT_LORA, LLAMA_EXAMPLE_LLAVA})); // WHRIA
    add_opt(common_arg(
